...
begin 
    ...
    begin
        ...
        r = permutedims(reshape(xtest,15,3,:),(2,1,3))
        RM = [0.36 0.48 -0.8; -0.8 0.6 0.0; 0.48 0.64 0.6]
        r = reshape(permutedims(reshape(RM*reshape(r,3,:),(3,15,:)),(2,1,3)),45,:).+40
        test_trans = (r,ytest)
        f2,m2,r2,err2 = validate(Eapprox,test_trans)
        @show m2,r2
        # display(plot(f1,f2,size=(800,300)))
        ...
        r = reshape(permutedims(reshape(RM*reshape(r,3,:),(3,15,:)),(2,1,3))[idx,:,:],(45,:)).+40
        test_trans = (r,ytest)
        f2,m2,r2,err2 = validate(Eapprox,test_trans)
        @show m2,r2
        # display(plot(f1,f2,size=(800,300)))
        ...
        file = h5open("logs/black_box/toluene_energy.hdf5","w")
        file["err"] = err2
    ...
    begin
        ...
        file = h5open("logs/black_box/toluene_force.hdf5","w")
        file["err"] = err2
        file["layers"] = layers[1]
        file["MAE"] = m2
        file["RMSE"] = r2
